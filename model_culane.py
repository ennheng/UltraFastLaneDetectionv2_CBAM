...
from model.resnet_cbam import ResNet, BasicBlock,Bottleneck
...
class parsingNet(torch.nn.Module):
    def __init__(self, pretrained=True, backbone='50', num_grid_row = None, num_cls_row = None, num_grid_col = None, num_cls_col = None, num_lane_on_row = None, num_lane_on_col = None, use_aux=False,input_height = None, input_width = None, fc_norm = False):
        ...
        mlp_mid_dim = 2048
        self.input_dim = input_height // 32 * input_width // 32 * 4
        print(self.input_dim)

        #self.model = resnet(backbone, pretrained=pretrained)
        #self.model = SEResNet(channels=[[64, 64], [128, 128], [256, 256], [512, 512]],init_block_channels=16,bottleneck=False,conv1_stride=True)
        self.model = ResNet(BasicBlock, [3, 4, 6, 3],num_classes=1000)
    ...
    def forward(self, x):
        ...
        #fea = self.pool(fea)
        #fea = torch.reshape(fea, (fea.size(0), fea.size(1), fea.size(2), fea.size(3)))
        #print(fea.shape)
...
def get_model(cfg):
    return parsingNet(pretrained = False, backbone=cfg.backbone, num_grid_row = cfg.num_cell_row, num_cls_row = cfg.num_row, num_grid_col = cfg.num_cell_col, num_cls_col = cfg.num_col, num_lane_on_row = cfg.num_lanes, num_lane_on_col = cfg.num_lanes, use_aux = cfg.use_aux, input_height = cfg.train_height, input_width = cfg.train_width, fc_norm = cfg.fc_norm).cuda()